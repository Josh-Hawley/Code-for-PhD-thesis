...
function [P, M, t_step, DiffYN, DiffYNflash, CT,DiffThresh, MM, HILL, grad]=diffSolver(P, M, Nt, TauND_step, TauH_step, NM, gamma, dm, dp, dm1, dm2, dp1, dp2, dt, Stochastic, rows, cols, DiffTime, S, ImplementSwapping, SwapThresh, Ts, PercentCounter, AvM, Replace, wl,a,b,P_ND0,n_ND,minT,maxT,na,nst,lines,dynamic, gradInput)
cells=rows*cols;
...
%     wl=wl_mins/dt; %Window length converted to number of vector elements
    ...
    if dynamic==1
        if mod(t_step-1,nst/dt)==0 %nst = noise switching time

            TauH = linspace(minT,maxT,rows)';
            if lines==1
                gn=na*randn(size(TauH)); %na = noise amplitude
                TauH=TauH+gn;
            end
...
%     EPS=1/6;
%     PSUM_DELAY=Psum_delay(2);
%     HILL(t_step)=a + b./(1+(EPS.*PSUM_DELAY./P_ND0).^n_ND);
    HILL(t_step)=1;
%     new1
